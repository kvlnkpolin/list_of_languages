...
Объяснение работы алгоритма: Функция bubble_sort принимает массив arr и определяет его длину n. Основной цикл for i in range(n) управляет проходами по массиву, используя флаг swapped для отслеживания обменов. Внутренний цикл for j in range(0, n-i-1) проходит по неотсортированной части, сравнивая соседние элементы через if arr[j] > arr[j+1]. При необходимости элементы обмениваются местами через множественное присваивание, а флаг swapped устанавливается в True. Если обменов не было, цикл прерывается досрочно. Функция print_array выводит элементы массива через пробел с помощью join. В основном блоке создается тестовый массив, выводится до и после сортировки.
