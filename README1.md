...
Анализ с точки зрения Big O: Время - O(log log n) в лучшем случае, O(n) в худшем случае
Память - O(1)

Причина временной сложности: В лучшем случае при равномерном распределении данных интерполяционная формула позволяет экспоненциально сокращать область поиска на каждом шаге. В худшем случае при неравномерных данных формула теряет эффективность, и алгоритм вырождается в последовательный перебор элементов.
